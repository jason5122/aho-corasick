#include "src/ac_fast.h"

#include "src/ac_slow.h"
...
static inline bool Binary_Search_Input(
    unsigned char* input_vect,
    int vect_len,
    char input,  // NOTE: This is the problematic piece of code!
                 // This should be `unsigned char`. Be absolutely sure type the conversions are
                 // correct.
    int& idx) {
...
ac_result_t Match(AC_Buffer* buf, std::string_view str, unsigned int len) {
    ...
    uint32* states_ofst_vect = (uint32*)(buf_base + buf->states_ofst_ofst);
